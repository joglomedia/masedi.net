...
class CMSC_Backup extends CMSC_Core
{
    ...
    function backup($args, $task_name = false)
    {
        ...
        if (trim($what) == 'db') {
            ...
            if ($db_result == false) {
                ...
                if (!$result) { // fallback to pclzip
                    ...
                    if ($disable_comp) {
                        $result = $archive->add($db_result, PCLZIP_OPT_REMOVE_PATH, CMSC_BACKUP_DIR, PCLZIP_OPT_NO_COMPRESSION);
                    } else {
                        $result = $archive->add($db_result, PCLZIP_OPT_REMOVE_PATH, CMSC_BACKUP_DIR);
                    }
                    @unlink($db_result);
                    @rmdir(CMSC_DB_DIR);
                    if (!$result) {
        ...
        if ($task_name) {
            ...
            //update_option('cmsc_backup_tasks', $backup_settings);
        }
        ...
        if ($task_name != 'Backup Now') {
            if (isset($account_info['cmsc_ftp']) && !empty($account_info['cmsc_ftp'])) {
                ...
                $ftp_result                             = $this->ftp_backup($account_info['cmsc_ftp']);
            ...
            if (isset($account_info['cmsc_amazon_s3']) && !empty($account_info['cmsc_amazon_s3'])) {
                ...
                $amazons3_result                              = $this->amazons3_backup($account_info['cmsc_amazon_s3']);
            ...
            if (isset($account_info['cmsc_dropbox']) && !empty($account_info['cmsc_dropbox'])) {
                ...
                $dropbox_result                             = $this->dropbox_backup($account_info['cmsc_dropbox']);
            ...
            if (isset($account_info['cmsc_email']) && !empty($account_info['cmsc_email'])) {
                ...
                $email_result = $this->email_backup($account_info['cmsc_email']);
    ...
    function backup_full($task_name, $backup_file, $exclude = array(), $include = array())
    {
        ...
        if (!$result) {
            ...
            if ($disable_comp) {
                $result_db = $archive->add($db_result, PCLZIP_OPT_REMOVE_PATH, CMSC_BACKUP_DIR, PCLZIP_OPT_NO_COMPRESSION);
            } else {
                $result_db = $archive->add($db_result, PCLZIP_OPT_REMOVE_PATH, CMSC_BACKUP_DIR);
            }
        ...
        $remove = array(
            ...
            trim(basename(WP_CONTENT_DIR)) . "/" . md5('mmb-worker') . "/cmsc_backups"
        );
        ...
        if ($backup_file && file_exists($backup_file)) {
            if ($overwrite) {
                ...
                if (trim($clone_from_url) || trim($cmsc_clone)) {
                    ...
                    $clone_options['_cmsc_action_message_id'] = get_option('_cmsc_action_message_id');
                   
                }
                 $clone_options['upload_path'] = get_option('upload_path');
                 $clone_options['upload_url_path'] = get_option('upload_url_path');
                 
                 $clone_options['cmsc_backup_tasks'] = serialize(get_option('cmsc_backup_tasks'));
                 $clone_options['cmsc_notifications'] = serialize(get_option('cmsc_notifications'));
                 $clone_options['cmsc_pageview_alerts'] = serialize(get_option('cmsc_pageview_alerts'));
    ...
    function email_backup($args)
    {
        ...
        if (file_exists($backup_file) && $email) {
            ...
            $headers     = 'From: CMS Commander <no-reply@cmscommander.com>' . "\r\n";
            $subject     = "CMS Commander - " . $task_name . " - " . $this->site_name;
            ob_start();
  ...
  function get_dropbox_backup($args){
    ...
    function remove_old_backups($task_name)
    {
        ...
        if ((count($backups[$task_name]['task_results']) - $num) >= $backups[$task_name]['task_args']['limit']) {
            ...
            //update_option('cmsc_backup_tasks', $backups);
        }
    }
    ...
    {
        $tasks             = $this->tasks;
        $backup_folder     = WP_CONTENT_DIR . '/' . md5('mmb-worker') . '/cmsc_backups/';
        ...
        if ((isset($files[0]) && basename($files[0]) == 'index.php' && count($files) == 1) || (empty($files))) {
            ...
            @rmdir(WP_CONTENT_DIR . '/' . md5('mmb-worker') . '/cmsc_backups');
    ...
    function remote_backup_now($args)
    {
        ...
        if ($backup_file && file_exists($backup_file)) {
            ...
            if (isset($account_info['cmsc_email']) && !empty($account_info['cmsc_email'])) {
                $account_info['cmsc_email']['file_path'] = $backup_file;
                $account_info['cmsc_email']['task_name'] = 'Backup Now';
