...
class CMSC_Helper
{
				...
				foreach($params['item_filter'] as $_items){
					if(!empty($_items)){
						foreach($_items as $_item){
							if(in_array($_item[0], $_cmsc_item_filter[$key])){
	...
	function cmsc_get_sitemeta_transient($option_name){
		...
		$result = $wpdb->get_var( $wpdb->prepare("SELECT `meta_value` FROM `{$wpdb->sitemeta}` WHERE meta_key = '{$option_name}' AND `site_id` = '{$this->cmsc_multisite}' ")); 
    ...
    function authenticate_message($data = false, $signature = false, $message_id = false)
    {
        ...
        if ((int) $current_message >= (int) $message_id)
            return array(
                'error' => 'Invalid message recieved. Deactivate and activate the CMS Commander plugin on this site, then remove the website from your CMS Commander account and add it again.'
            );
        
        $pl_key = $this->get_master_public_key();
        if (!$pl_key) {
            return array(
                'error' => 'Authentication failed. Deactivate and activate the CMS Commander plugin on this site, then remove the website from your CMS Commander account and add it again.'
            );
        }
        ...
        if (function_exists('openssl_verify') && !$this->get_random_signature()) {
            ...
            if ($verify == 1) {
                ...
                return array(
                    'error' => 'Invalid message signature. Deactivate and activate the CMS Commander plugin on this site, then remove the website from your CMS Commander account and add it again.'
                );
            } else {
                return array(
