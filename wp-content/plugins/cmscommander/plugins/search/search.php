...
if(basename($_SERVER['SCRIPT_FILENAME']) == "search.php"):
	...
	cmsc_add_action('cmsc_search_posts_by_term', 'cmsc_search_posts_by_term');
	
    function cmsc_search_posts_by_term($params = false){
    	...
    	case 'plugin':
    		...
    		if($have_plugin){
    			cmsc_response($have_plugin, true);
    		}else{
    			cmsc_response('Not found', false);
    		}
    		break;
    	case 'theme':
    		...
    		if(strpos(strtolower($tm_name), $search_term)>-1){
    				$have_theme[] = $tm_name; 
    				cmsc_response($have_theme, true);
    		}else{
    			cmsc_response('Not found', false);
    		}
    		break;
    	default: cmsc_response('Not found', false);
    	}
    }
