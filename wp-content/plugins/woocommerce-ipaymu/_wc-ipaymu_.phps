 ...
 * Description: WooCommerce Ipaymu Gateway is a fully-working Indonesia Payment Gateway (Ipaymu) for WooCommerce Wordpress store.
 * Author: MasEDI
...
function ipaymu_gateway_init() {
	 ...
	 **/
	class WC_Ipaymu_Gateway extends WC_Payment_Gateway {
		...
		**/
		public function __construct() {
			...
			// Define Paypal setting variables (Opsi pembayaran lain Ipaymu).
			...
			// Add actions.
			add_action('init', array(&$this, 'check_api_response'));
			add_action('valid_ipaymu_api_request', array(&$this, 'successful_request'));
			...
			//$this->enabled = ('yes' == $this->settings['enabled']) && !empty($this->ipaymu_apikey) && $this->is_valid_for_use();
		...
		**/
		...
		function init_form_fields() {
			$this->form_fields = array(
				...
				'description' => array(
					...
					'default' => __('Pay via Ipaymu. Alternatively you can pay with your PayPal account instead of Ipaymu, you can also pay with credit card if you don\'t have Ipaymu or PayPal account', 'wcipaymu')
				),
		...
		function get_ipaymu_args($order) {
			...
			if ($this->debug=='yes') $this->log->add('wcipaymu', 'Generating Ipaymu payment args for order #' . $order_id . '. Notify URL: ' . trailingslashit(home_url()).'?ipaymuListener=notify');
			...
			// cari metode utk menyertakan $order_key dikirim ke gateway dan dikirim balik lewat unotify!!! masukan sbg parameter pada unotify url
				...
				if($currency_code <> 'USD') {
					...
					if(! empty($usd_currency_rate)) {
						...
						if($cached_usd_currency_rate !== $usd_currency_rate) {
							...
							// cache currency rate setting to avoid continuosely grabbing from bca (for performance, cache min 24 hrs)
						...
						// convert the total price in IDR to USD using setting currency rate
						$price_usd = $total_price / $usd_currency_rate;
					} else {
						// get cached currency rate
						$cached_usd_currency_rate = get_transient($this->id . '_currency_rate');
						
						if($cached_usd_currency_rate) {
		...
		* @return string
		...
		function generate_ipaymu_form($order_id) {
			...
			$response = $this->send_to_gateway($this->gateway_url, $args, 'POST');
		 ...
		 **/
		function send_to_gateway($url, $params='', $method='POST', $ua = '', $cookie = false, $proxy = false, $proxyauth = false, $timeout = 30) {
			// set user agent
		 ...
		 * @return void
		 **/
		function check_api_response_is_valid($data) {
			global $woocommerce;
		...
		function check_api_response() {
			...
			if (isset($_GET['ipaymuListener']) && $_GET['ipaymuListener'] == 'notify') {
				...
				// logs
				if ($this->debug == 'yes') $this->log->add('wcipaymu', 'Receiving payment response from Ipaymu Gateway: ' . print_r($_REQUEST, true));
				...
				$data = $this->check_api_response_is_valid($_POST);
				
				if ($data) {
					header('HTTP/1.1 200 OK');
					do_action('valid_ipaymu_api_request', $_POST);
				} else {
		...
		function successful_request($posted) {
			...
			// backward comp for API request, check the order ID based on gateway response args
			...
			//if ($order->id === $order_id) {
				...
				// If returned order_key not equal with current order_key
				...
				if ($trx_id <> '') {
					...
					if (!$payment) {
						// logs
						if ($this->debug == 'yes') $this->log->add('wcipaymu', 'Trouble error checking payment status from Ipaymu Gateway. ' . print_r($payment, true));
					} else {
						// logs
						...
						switch ($payment['Status']) :
							...
							case "3":
								...
								$ipaymu_fee = $order_total * 0.01; // ipaymu fee is 1% of total paid amount and not refundable, so we need to add it to the refunded balance
		...
		/**
		 * Checking for Ipaymu Payment Staus.
		 *
		 * @access public
		...
		function check_payment_status($trx_id) {
			...
			$response = $this->send_to_gateway($this->gateway_trx_url, $args, 'POST');
		...
		**/
		...
		function masedi_credits_link() {
			...
			if($this->credit_link == 'yes') {
				$credits = '<div class="credit">' . get_bloginfo('name') . ' uses <a href="http://masedi.net/plugin-ipaymu-payment-for-woocommerce-indonesia.html" target="_blank" title="Download Ipaymu Payment Gateway for WooCommerce">Ipaymu for WooCommerce plugin</a> developed by <a href="http://masedi.net/" target="_blank" title="Online Store Developer">MasEDI.Net</a></div>';			
				echo $credits;
			}
		}
		 ...
		 * $output output format: JSON, XML
		 **/
		function get_kurs_bca($kursurl, $currency) {
			$bca_rates = array();

			$response = $this->send_to_gateway($kursurl, '', 'GET'); // get result Object array status, rawdata
	 ...
	 **/
	if (! function_exists('add_idr_currency')) {
		function add_idr_currency($currencies) {
			$currencies['IDR'] = __('Indonesia Rupiah (Rp)', 'wcipaymu');	
			return $currencies;
		}
		add_filter('woocommerce_currencies', 'add_idr_currency');
	}

	if (! function_exists('add_idr_currency_symbol')) {
		function add_idr_currency_symbol($currency_symbol, $currency) {
			switch($currency) {
				case 'IDR': $currency_symbol = 'Rp.'; break;
			}
			return $currency_symbol;
		}
		add_filter('woocommerce_currency_symbol', 'add_idr_currency_symbol', 10, 2);
	}
	
	// additional currencies for Gravity Forms, basically should not here, but very useful for store that using gravity form as addons
