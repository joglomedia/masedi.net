...
$catsql = "select * from $wpdb->terms c,$wpdb->term_taxonomy tt  where tt.term_id=c.term_id and tt.taxonomy='".CUSTOM_CATEGORY_TYPE2."' and tt.parent=0 and c.name != 'Uncategorized' and c.name != 'Blog'  $substr order by c.term_id";
...
if($catinfo) {
	...
	$counter = 0;
	?>
		<div class="form_cat" ><label><input type="checkbox" name="selectall" onclick="displaychk();" id="selectall" /><?php echo SELECT_ALL;?></label></div>
	<?php
...
function displaychk(){
	...
	if(document.resumeform.selectall.checked == true) {
		for (i = 0; i < len; i++)
		chk[i].checked = true ;
	} else {
		for (i = 0; i < len; i++)
