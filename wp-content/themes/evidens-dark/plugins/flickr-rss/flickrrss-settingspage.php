	...
	<form method="post">
		<table class="form-table">
			...
			<tr valign="top" id="userid">
				...
				<td><input name="flickrRSS_id" type="text" id="flickrRSS_id" value="<?php echo $settings['id']; ?>" size="20" />
					<a href="#" id="idgetter">Click here if you don't know it</a></td>
			</tr>
				...
				<td>
					<table>
						...
						<tr>
							...
							<td valign="top" style="border-width: 0px;">
								<div>
									<h4>"Item HTML" valid metatags:</h4>
									<ul>
										...
										<li><code>%image_large%</code> (Warning: not always available)</li>
									</ul>
								</div>
							</td>
						</tr>
		...
		<table class="form-table">
			...
			<tr valign="top" class="cachesettings">
				...
				<td>
					<?php
						foreach (array("small", "square", "thumb", "medium", "large") as $size) {
		...
		<div class="submit">
			<input type="submit" name="reset_flickrRSS_settings" value="<?php _e('Restore Default Settings') ?>" />
			<input type="submit" name="save_flickrRSS_settings" value="<?php _e('Save Settings') ?>" />
		</div>
		<script>
			(function() {
				var $ = jQuery;
